...
二期在此基础上增加通用策略、事中监管、告警、成员访问审批、限时特例和网页水印。
...
| 部门管理员 | 在授权部门与可管理环境权限边界内管理策略与申请；成员/环境并存时按并集覆盖 |
...
网页采集插件属于一期网页资源管理能力：管理员在指纹浏览器账号环境内通过悬浮面板采集网页或元素，客户端生成脱敏采集草稿，服务端保存资源版本，管理端确认后再被访问策略引用。
...
| `*`、全部成员、全部环境或成员+环境并集等高影响组合 | 二次确认并记录风险确认 |
| 系统硬拦截被尝试改为可申请 | 阻止发布 |

#### 作用范围计算

成员范围和环境范围是两个独立的策略覆盖维度。发布预览和运行时决策都按以下规则计算：

```text
成员命中 = 当前成员属于策略成员范围
环境命中 = 当前环境属于策略环境范围
策略范围命中 = 成员命中 OR 环境命中
策略最终命中 = 策略范围命中 AND 生效时间命中 AND 网页/动作规则命中
```

服务端先将两个维度分别限制在管理员可管范围内，再展开为会话集合取并集；成员维度不得跨出管理员可管环境，环境维度不得扩展到管理员不可管成员。未启用的维度不产生命中，显式选择“全部成员”或“全部环境”才代表该维度的管理员可管全量。并集不改变成员原有环境授权。若策略同时选择成员 A 和环境 B，则 A 在其他已授权且管理员可管的环境中的访问、其他有权使用 B 且管理员可管的成员访问都会进入该策略覆盖范围，但成员和环境均未命中的会话不会命中该策略。

预览结果必须同时返回成员维度覆盖量、环境维度覆盖量、去重后的并集覆盖量和新增覆盖会话；事件记录 `scope_hit_source`，标识本次命中来自成员、环境或两者。二期访问特例仍按申请绑定的成员、环境、网页、动作和有效期精确匹配，不继承基础策略的并集扩大范围。
...
flowchart TD
    ...
    C -- 是 --> D[BLOCK]
    C -- 否 --> S{成员或环境范围命中?}
    S -- 否 --> T[跳过当前策略，继续评估其他策略]
    S -- 是 --> E[收集时间、网页资源和动作均匹配的规则]
    E --> F{是否命中禁止访问规则?}
    F -- 是 --> D
    F -- 否 --> G{是否处于白名单范围?}
...
| 2 | 安全管理员 | 成员范围选择“全部成员”，排除“财务”角色；环境范围选择全部 Amazon 环境 |
...
1. 管理员在“收款账户设置”页面打开浏览器内置悬浮采集面板；
2. 选择“网页采集”保存页面标题和规范化 URL，或切换“元素采集”选择“新增银行卡”和“更换收款账户”按钮；
3. 客户端完成脱敏和定位器预览，服务端保存为网页资源草稿；
4. 管理员在网页资源管理中测试匹配并确认资源，再回填访问策略；
5. 在黑名单策略中允许查看页面，但配置禁止查看或禁止点击上述元素，并开启访问记录；
6. 成员打开页面后，客户端识别并限制元素；元素限制成功写入 `ELEMENT_RESTRICTED`；
7. 页面改版导致元素未找到时写入 `ELEMENT_NOT_FOUND`，管理端显示资源健康异常；
8. 管理员判断该风险不能接受元素失效时，将网页策略改为整页禁止。
...
1. 管理员创建黑名单策略，选择成员 A，同时选择环境 B；
2. 成员 A 在已授权环境 C 中访问目标网页，因成员命中而纳入策略；
3. 成员 D 在环境 B 中访问目标网页，因环境命中而纳入策略；
4. 成员 D 在未选择且未被其他条件命中的环境 C 中访问，不因该策略命中；
5. 日志分别记录成员命中、环境命中或两者同时命中的来源，预览展示去重后的并集覆盖量。
...
| AC-07 二期通用策略 | 二期成员被禁止使用开发者工具 | 尝试打开 | 动作被阻止且记录元数据 |
...
| AC-19 成员与环境并集 | 策略同时选择成员 A 和环境 B | A 访问 C，D 访问 B，D 访问 C | A/C 与 D/B 均命中；D/C 不因该策略命中；日志记录命中来源 |
| AC-20 并集不改变授权 | D 没有环境 B 的原有授权 | D 尝试打开 B | 先按现有环境授权拒绝，不因策略环境范围选择而获得访问权 |
...
2. 二期通用策略是否确认包含复制、上传和下载；本文按包含处理。
