...
```text
一期：访问控制底座
黑/白名单 + 页面访问控制 + 元素限制 + 访问日志 + 网页资源管理（含网页采集插件）

二期：安全监管闭环
通用策略 + 事中监管 + 告警方案 + 监管日志 + 成员访问审批 + 网页水印
...
范围基线：成员范围和环境范围可以独立配置；两者同时配置时按并集覆盖，成员命中或环境命中任一维度即可进入策略范围，但不改变成员原有的环境授权。
...
- **最小采集**：一期网页采集插件不采集页面正文、输入值、截图、Cookie、Token 或文件原文。
...
|---|---|---|---|---|
| 一期 V1.0 | 访问控制底座 | 管理员能配置并验证网页与元素访问限制 | 访问策略管理、访问日志、网页资源管理 | 高危页面可稳定拦截；黑/白名单冲突可解释；事件可追溯 |
...
成员范围和环境范围是两个可以独立配置的覆盖维度。策略可以只配置其中一个，也可以同时配置两个；两者同时配置时按并集计算，不要求同一次访问同时满足成员条件和环境条件。

```text
成员维度覆盖 = 选定成员在管理员可管且其已授权环境内的访问会话
环境维度覆盖 = 选定环境内、管理员可管且具有该环境使用权的成员访问会话
策略最终覆盖 = 成员维度覆盖 ∪ 环境维度覆盖
```

具体规则：

1. 只配置成员时，策略覆盖这些成员在管理员可管且已有环境授权内的会话。
2. 只配置环境时，策略覆盖管理员可管环境内所有有权使用这些环境的成员会话。
3. 同时配置成员和环境时，成员命中或环境命中任一条件即可进入策略覆盖范围；不要求成员和环境同时命中。
4. 成员、部门、角色等成员条件，以及环境组、指定环境、平台账号等环境条件，在各自维度内先合并计算；排除条件只从对应维度集合中扣除。
5. 并集只决定安全策略是否覆盖会话，不授予、回收或改变成员原有的环境授权；实际会话仍必须先通过现有环境授权校验。
6. 服务端先将成员范围和环境范围分别限制在管理员可管边界内，再展开为会话集合并计算并集；成员维度不得跨出管理员可管环境，环境维度不得扩展到管理员不可管成员。
7. 未启用某一维度时，该维度不产生命中；只有显式选择“全部成员”或“全部环境”时，才将该维度展开为管理员可管的全量范围。

例如，策略同时选择成员 A 和环境 B：成员 A 在其已授权且管理员可管的环境 C 中访问时会被覆盖；成员 D 在环境 B 中访问时也会被覆盖；成员 D 在未被选择的环境 C 中访问时不因该策略命中。

发布前预览必须分别展示成员维度覆盖量、环境维度覆盖量、去重后的并集覆盖量和由并集新增的会话数量；当任一维度选择“全部”时，需要提示可能覆盖全部已授权会话。
...
二期通用策略不依赖某个网页资源，按成员/部门/角色及环境范围生效：
...
| 成员 | 事件时成员、部门和角色快照 |
| 环境/平台账号 | 事件时环境和账号快照 |
| 操作类型 | 一期页面访问、元素查看/点击；二期扩展密码、开发者工具、复制、上传、下载、打印 |
| 网页名称/URL | 资源名与脱敏 URL |
| 触发策略 | 策略名称、版本和规则 |
...
网页采集插件纳入一期网页资源管理：采用指纹浏览器内置悬浮面板形态，支持右键菜单、浏览器工具栏图标、页面悬浮图标和管理端策略向导四类入口。插件提供“网页采集”和“元素采集”两种模式，前者获取网页标题与规范化 URL，后者获取元素名称、定位器候选和页面版本。

插件数据链路为“页面 DOM/URL -> 客户端采集适配层 -> `capture_draft` -> 服务端资源草稿 -> 网页资源管理测试与发布 -> 访问策略引用 -> 客户端策略包”。插件只能创建采集草稿，不能直接发布策略；客户端完成本地脱敏和定位器预览，服务端完成鉴权、敏感参数清理、团队隔离、资源版本和幂等保存，管理端完成确认、编辑、匹配测试和发布。
...
- 成员与环境同时配置时，管理端预览、服务端决策和客户端执行均按“成员命中或环境命中”的并集规则得到一致结果。
...
- 基础策略的成员/环境并集只决定是否进入策略覆盖范围；访问特例仍必须同时匹配申请绑定的成员、环境、网页资源、动作和有效期，不得因基础策略采用并集而扩大。
- 审批通过创建限时访问特例，不修改原策略；
...
| Gate | 通过条件 |
|---|---|
| G0 技术可行性 | 页面访问和一期元素控制具有明确可控范围；不可控路径写入非目标和风险提示 |
| G1 规则一致性 | 管理端预览、服务端决策、客户端执行对成员/环境并集及网页规则使用同一匹配测试集 |
...
| 成员与环境并集配置造成覆盖范围扩大 | 一期 | 预览分别展示两个维度和去重并集，提示新增覆盖会话，并提供灰度、停用与回滚 |
...
2. 二期通用策略是否确认包含复制、上传和下载；本文除密码、开发者工具、打印外，也纳入这三项。
