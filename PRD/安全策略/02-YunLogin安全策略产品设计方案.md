...
YunLogin 新增一级模块“安全策略”，以“访问策略管理”为一期核心，复用现有团队成员、部门、角色、环境、平台账号和日志能力。一期完成：

1. 管理员基于系统模板或自定义资源创建黑名单或白名单策略；
2. 策略作用于成员/部门/角色与环境/环境组/平台账号，可永久或定时生效；
3. 每个网页可配置允许/禁止访问、是否记录访问行为和限制网页元素；
4. 每次决策记录策略版本、网页/元素资源、客户端和执行结果；
5. 管理端可验证哪些客户端已同步并具备执行能力。
...
在 SystemFrame 左侧新增一级菜单“安全策略”，位于“团队”之后、“插件管理”之前。一级菜单下首期只建设三个页面，通用策略和访问审批不提前占用导航入口。
...
二期能力（暂不作为一期一级菜单）：通用策略、访问审批、事中监管、告警方案、监管日志、网页水印
后续能力：安全概览
```
...
| 环境安全监控 | 二期迁移到“安全策略 > 事中监管” | 目前仅有开关概念，没有监控对象、事件和生效规则 |
...
- 部门管理员只能管理授权部门与可管理环境权限共同覆盖的数据；服务端先完成权限边界校验，再按成员/环境并集计算策略覆盖；
...
| `PolicyScope` | 成员/部门/角色与环境/环境组/平台账号的作用范围，成员和环境维度按并集命中 |
...
| `PolicyBundle` | 下发客户端的签名策略包，按团队及成员/环境范围裁剪 |
...
成员范围和环境范围是两个可独立配置的覆盖维度。策略配置时可以同时选择成员和环境；两者并存时按并集命中，不要求同一次访问同时满足两个维度。
...
S_member = {(m, e) | m ∈ M 且 e ∈ E_admin 且 m 对 e 仍有环境授权}
S_environment = {(m, e) | m ∈ M_admin 且 e ∈ E 且 m 对 e 仍有环境授权}
S_policy = S_member ∪ S_environment
```

若某一维度未启用，对应集合为空；只有显式选择“全部成员”或“全部环境”时，才将该维度展开为管理员可管的全量集合。

因此：只配置成员时，覆盖这些成员在管理员可管且已有授权的环境内的会话；只配置环境时，覆盖管理员可管环境内、具有对应环境使用权的成员会话；同时配置成员和环境时，成员命中或环境命中任一条件即可进入策略覆盖范围。并集只决定安全策略覆盖，不授予或改变环境授权。成员、部门、角色的多选项，以及环境组、指定环境、平台账号的多选项，在各自维度内先合并，排除条件再从对应集合中扣除。

成员变更部门/角色、环境换绑账号或授权关系变化时实时重新计算，不把成员 ID 清单固化在已发布版本中；事件仍保存命中时的组织与环境快照，并记录 `scope_hit_source`（`MEMBER`、`ENVIRONMENT` 或 `BOTH`）。
...
- 成员维度覆盖量、环境维度覆盖量、去重后的并集覆盖量，以及由并集新增的会话数量；
- 与现有策略的重叠和最终决策；
- 客户端版本不支持的目标会话；
...
| 智能网页采集 | 一期 | 从指纹浏览器内置悬浮采集面板批量采集当前页面规范化 URL；采集不自动发布策略 |
...
网页采集插件是指纹浏览器客户端内置的“网页资源采集面板”，不是独立安装的第三方扩展。它只在管理员打开具备采集权限的账号环境时可用，默认以悬浮图标收起，点击后展开窄面板，不遮挡浏览器主工作区；面板支持拖拽位置、收起和重新打开。

#### 入口与采集模式

| 入口 | 触发条件 | 结果 |
|---|---|---|
| 右键菜单 | 在页面中右键并选择“采集网页资源” | 以当前页面为上下文打开采集面板 |
| 浏览器插件图标 | 点击指纹浏览器工具栏采集图标 | 打开当前页采集面板 |
| 页面悬浮图标 | 页面加载完成且用户拥有采集权限 | 打开当前页采集面板 |
| 管理端策略向导 | 在网页资源选择器点击“从当前环境采集” | 打开面板并在保存后回填策略草稿 |

面板提供两种模式：

1. **网页采集**：采集当前页面标题、规范化 URL、平台/站点上下文和页面版本，可连续添加多个页面；智能采集模式可捕获同一业务流程中变化的 URL，并过滤客户端自身域名。
2. **元素采集**：进入元素选择态后高亮鼠标所在元素，显示元素名称、定位器预览和候选匹配数量；管理员确认后保存“禁止查看”或“禁止点击”所需的元素资源。
...
数据对象分层：插件只提交 `capture_draft`，不能直接发布策略；客户端负责采集和本地校验，服务端负责鉴权、脱敏、资源版本和团队隔离，管理端负责确认、编辑和发布。采集面板与客户端之间采用本地受控消息通道，消息携带 `capture_session_id`、`team_id`、`environment_id`、`page_url`、`page_title`、`elements[]` 和 `client_version`，服务端写入资源草稿后返回 `resource_id` 与版本号。

安全边界：默认不采集页面正文、输入值、Cookie、Token、密码、截图和文件原文；URL 去除 fragment、凭据及敏感 query。采集失败、权限失效、页面跨域不可读、元素多匹配或定位器不稳定时，面板必须展示失败原因，不得生成“看似成功”的资源。

#### 插件验收标准

- 采集面板只能在指纹浏览器账号环境内打开，普通成员和无网页资源管理权限用户不可见；
- 网页采集结果可编辑名称、平台、站点、风险等级和 URL 匹配选项；
- 元素采集至少支持 CSS/文本语义/属性组合定位器预览，并展示候选数量和健康状态；
- “保存至网页资源管理”和“保存并回填访问策略”是两个明确动作，前者不自动创建策略；
- 面板关闭、网络失败或客户端升级时，未提交草稿可本地暂存并明确标记，不能静默丢失；
- 采集资源必须经过服务端匹配测试后才能被发布策略引用。
...
常规规则的范围命中条件为“成员命中 **或** 环境命中”，再叠加生效时间、网页资源和动作条件：

```text
规则候选 = (member_scope_hit OR environment_scope_hit)
        AND time_hit
        AND resource_or_action_hit
```

未启用的维度不产生命中，显式选择“全部成员”或“全部环境”才按当前管理员可管全量处理；并集只决定规则是否进入候选集合，黑名单/白名单冲突、系统硬拦截和二期精确特例仍按各自优先级继续决策。决策结果应返回并记录 `scope_hit_source`，便于管理员解释“因成员命中、环境命中还是两者同时命中”。
...
3. 收集成员或环境作用域命中，且时间、网页资源和动作均匹配的常规规则
...
```json
{
  ...
  "scope_hit_source": ["MEMBER", "ENVIRONMENT"],
...
2. 编译为按团队及成员/环境范围裁剪的 `PolicyBundle`；
...
- 元素查看/点击就地限制，并使用 Toast/Popover 说明；
- 二期允许申请时再提供“申请临时操作权限”；
...
| 环境/平台账号 | 事件时环境和账号快照，成员离职或环境删除后仍可解释 |
...
| 网页水印 | 不在二期通用限制首发 | 二期监管按成员、管理端/环境场景和内容模板配置 |
...
7. 多策略冲突时结果符合固定优先级，日志可解释全部命中规则；
8. 客户端过旧、策略包无效或关键策略未同步时不能静默进入受控环境；
9. 历史策略版本和安全事件不可由业务界面删除或篡改。
10. 成员与环境同时配置时，指定成员在其他已授权环境中的会话、其他成员在指定环境中的会话均被覆盖；两者均未命中的会话不被该策略覆盖。
