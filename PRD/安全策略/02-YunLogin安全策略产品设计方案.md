...
YunLogin 新增一级模块“安全中心”，以“访问策略”为首期核心，复用现有团队成员、部门、角色、环境、平台账号和日志能力。首期完成：

1. 管理员基于系统模板或自定义 URL 创建敏感网页策略；
2. 策略作用于成员/部门/角色与环境/环境组/平台账号，可永久或定时生效；
3. 成员命中后被拦截，看到安全化原因并可按策略申请临时访问；
4. 审批通过产生限时访问特例，到期自动失效；
5. 每次决策记录策略版本、规则、客户端和结果，安全管理员可调查；
6. 管理端可验证哪些客户端已同步并具备执行能力。

方案采用“系统硬拦截 > 有效访问特例 > 常规策略严重度 > 默认允许”的确定性决策，不采用“最近编辑的策略获胜”。
...
| 环境安全监控 | 迁移到“安全中心 > 通用限制/安全概览”后再明确能力 | 目前仅有开关概念，没有监控对象、事件和生效规则 |
| 高风险操作提醒 | P1 纳入安全通知规则 | 提醒依赖可识别的事件，不应是无来源的独立开关 |
...
- 审批人不能审批本人申请；如策略只有本人可审批，系统必须在发布前阻止；
...
| `security:approval:view` | 查看授权范围内访问申请 |
| `security:approval:decide` | 审批非本人申请 |
...
| `AccessRequest` | 成员对一次命中发起的临时访问申请 |
| `AccessException` | 审批产生的限时、限成员、限环境、限资源放行凭证 |
...
| 处置 | 说明 | 是否生成事件 |
|---|---|---|
| 允许并记录 `AUDIT` | 页面正常访问，记录元数据 | 是 |
| 需要审批 `REQUIRE_APPROVAL` | 无有效特例时拦截，可申请临时访问 | 是 |
| 禁止访问 `BLOCK` | 直接拦截；可配置是否允许申请 | 是 |

P1 增加 `WARN` 和操作级规则。处置严重度为：`BLOCK > REQUIRE_APPROVAL > WARN > AUDIT > ALLOW`。

#### P0 页面动作

- `NAVIGATE`：地址栏输入、点击链接、书签、新窗口、服务端重定向产生的顶层页面访问；
- 对弹窗和新窗口在提交导航前执行相同判断；
- iframe、XHR/fetch、WebSocket 和 Service Worker 请求不在 P0 的页面访问保证范围内，避免把 URL 拦截宣传成完整 DLP。

#### P1 通用动作

- `COPY`、`UPLOAD`、`DOWNLOAD`、`PRINT`；
- `DEVTOOLS`、`VIEW_CREDENTIAL`；
- `ELEMENT_CLICK` 为 P2，需页面元素资源健康检查。

P0 先把查看密码、开发者工具和打印作为按成员/角色生效的团队通用限制接入，保持配置简单；P1 再把这些动作升级为可按环境、网页资源和时段配置的 `PolicyRule`。两类配置最终进入同一决策与日志模型，不能形成两套互不解释的开关。

### 5.4 第四步：申请与审批

| 字段 | 规则 |
|---|---|
| 是否允许申请 | 允许 / 禁止；关键系统硬拦截只允许禁止 |
| 审批人 | 团队创建者、安全管理员、指定角色或指定成员；不能只包含策略目标本人 |
| 申请理由 | 必填，10–300 字 |
| 申请时长 | 15 分钟、30 分钟、1 小时、2 小时、自定义；建议默认 30 分钟，最大 24 小时 |
| 审批方式 | 单人审批；高风险可选双人会签（P1） |
| 生效方式 | 立即生效 / 指定未来开始时间；到期自动失效 |
| 放行范围 | 固定为申请成员 + 当前环境 + 当前网页资源；不可把一次申请扩大到全团队 |

审批通过后产生 `AccessException`，不是修改或停用原策略。审批记录与原策略版本永久关联。
...
| 智能采集 | P1 | 从环境内插件采集当前页面规范化 URL；采集不自动发布策略 |
| 元素资源 | P2 | 保存元素定位、多语言/多站点变体和健康状态 |
...
```text
1. 系统硬拦截命中                 -> BLOCK（不可申请）
2. 有效 AccessException 精确命中  -> ALLOW + AUDIT
3. 收集全部作用域、时间和资源均命中的常规规则
4. 取最高严重度                   -> BLOCK / REQUIRE_APPROVAL / WARN / AUDIT / ALLOW
5. 无匹配规则                     -> 默认 ALLOW
```
...
P1 白名单不是“比黑名单更高的允许规则”，而是把某个范围的默认结果从 `ALLOW` 改为 `BLOCK`：
...
因此提现页即使位于允许域名内，仍可被更具体的高危黑名单拦截。只有有效临时特例可以放行普通拒绝规则，避免竞品“白名单优先”造成误放行。
...
- P0 不承诺阻止已加载页面通过 XHR 调用高危接口，需在运营平台支持时用元素/动作规则或平台自身权限补强。
...
| 最终结果 | 允许、允许并记录、警告、要求审批、已拦截、特例放行、执行失败 |
| 触发策略 | 策略名与不可变版本 |
...
P0 提供团队级基线，成员范围支持全部、指定成员、部门和角色：
...
| 扩展管理 | 展示既有权限入口和风险说明，不在首期新增拦截 | 禁止安装/卸载非受信扩展 |
| 屏幕水印 | 不在 P0 | 按成员、管理端/环境场景和内容模板配置 |
...
5. 允许申请时成员可提交理由，审批人通过后仅在指定范围和时长内放行；
6. 到期或撤销后，下一次访问重新拦截，无需管理员手工停用原策略；
7. 多策略冲突时结果符合固定优先级，日志可解释全部命中规则；
8. 客户端过旧、策略包无效或关键策略未同步时不能静默进入受控环境；
9. 创建者命中策略时同样被拦截，只有合规紧急解锁可例外；
10. 历史策略版本、审批和安全事件不可由业务界面删除或篡改。

### 17.2 安全与隐私验收

- 页面正文、密码、Cookie、Token 和文件正文不进入 P0 日志；
- URL 敏感参数默认脱敏；
- 普通成员不能查看其他成员申请或日志；
- 审批人不能审批本人申请；
