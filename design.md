...
- 订单管理数据区与工具栏共用同一 `bg-card` 白色区块，卡片使用 8px 圆角和 `p-5 md:p-6` 内边距；表格外层必须使用 `overflow-x:auto`。
- 订单表格最小宽度为 1180px，字段顺序固定为：订单号、订单类型、商品名称、订单金额、应付金额、支付方式、支付时间、订单状态、创建人、创建时间、操作。所有表头和单元格左对齐，数字、金额、订单号和时间使用 JetBrains Mono。
- 订单号为可点击文本，默认使用 `ink-body`，hover/focus-visible 切换为 `primary` 并显示下划线；点击订单号和操作列“详情”打开同一 SystemFrame 订单详情抽屉。
- 订单状态只使用语义色纯文本，不使用背景、边框、圆角或 Badge 容器；颜色映射为：已完成使用 `success`，创建中/处理中/开通中使用 `info`，待支付/已退款/部分退款使用 `warning`，异常使用 `danger`，已关闭使用 `ink-muted`。
- 表格行高度约 44–52px，数据行下边框使用 `line-lighter`，最后一行也必须保留下边框；hover 行使用 `bg-hover`，不得用最后一行选择器移除边框。
- 订单金额、应付金额、支付时间和创建时间支持原生按钮式升降序排序。排序图标固定为 `10×14px` 上下双三角，字段名与当前方向同步使用 `primary`；所属 `th` 必须同步 `aria-sort`。首次进入按创建时间降序，切换字段时该字段首次按降序，空支付时间始终置底。
- 操作列表头、单元格和操作组均左对齐，多个操作使用 `display:flex;align-items:center;justify-content:flex-start;gap:12px;white-space:nowrap`，不得用逐个链接的 margin 拼接间距。
...
- 开票申请 Modal 为业务特例：所有字段统一使用 112px 标签列和 400px 控件列，标签 `justify-self:end` 右对齐、控件 `justify-self:start` 左对齐且起点一致；标签使用 `white-space:nowrap`，移动端也不切换为左对齐，控件列在可用宽度内收缩。字段标签只保留一个中文全角冒号，不同时使用文本冒号和伪元素；非必填字段不在标签中重复展示“选填”。发票抬头采用文本输入框 + 单选下拉，输入框、选项面板和 `×` 清空按钮作为同一控件列内组合，不显示独立编辑按钮，也不改变其他字段起点。
- 开票申请第二步校验错误采用字段级就地提示：危险色文字紧贴对应控件下方并占用控件列宽度，禁止把多个错误集中渲染在表单左下角；首个错误字段获得焦点，修正后清除该字段错误。
...
- 打开后必须锁定背景滚动、将焦点移入面板并约束 Tab；支持关闭按钮、点击遮罩和 `Escape` 关闭，关闭后恢复触发控件焦点。业务内容必须以同一个稳定面板引用成对调用 `pushAnnoScope()` / `popAnnoScope()`。
...
- 同一页面只允许一个会话窗；关闭按钮和 `Escape` 均可关闭并恢复触发入口焦点。打开期间必须限定在会话窗内部。
- 输入区不展示实时字数计数；模块可保留内部长度校验。会话消息区使用 `role=log` 仅追加新消息，并区分客服与用户消息语义。工具栏评价入口使用 Lucide `star` 图标，打开客服窗内嵌套评价层，不增加页面级遮罩。
- 评价层沿用 `card`、8px 圆角和中层阴影，提供四档 `frown / meh / smile / heart` Lucide 图标、可选意见输入和 `success` 提交按钮；选项使用 `aria-pressed` 表达互斥状态，打开评价层时切换至评价层内部。
...
- 成功/信息提示使用 `role="status"` 与 `aria-live="polite"`，错误提示使用 `role="alert"`；展示时不得强制转移焦点。Toast 层级高于普通业务弹层，但低于交互。
...
> 通过 渲染，与页面 DOM 完全解耦。详见 `claude.md` §7。
...
**标准悬浮开关**：使用“tags 图标 + 状态文案”的 32px 高胶囊按钮，右侧间距 8px。，开关默认文案为“显示标注”；单击后显示标注并将文案切换为“隐藏标注”，再次单击恢复默认隐藏状态。
...
- SystemFrame 独占 BrowserChrome、TopBar、Sidebar、全局助手、全局 Toast / Popover / Dialog / Drawer 与全局标注；这些元素相对顶层视口定位。业务模块默认只渲染业务内容、业务弹层与业务内容，业务弹层相对 iframe 视口定位、不跨越 Sidebar 或 TopBar；模块 PRD 明确要求全局遮罩的业务详情属于受控例外，只能通过结构化消息调用 SystemFrame 的全局 Drawer Portal，不得传 HTML、复制模块 DOM 或直接修改父页面；
...
- 侧栏一级、二级、可展开分组、“新建浏览器”及底部入口名称统一使用 14px；底部固定入口依次为“设置”（`settings-2`）与“帮助”（`circle-help`），分别进入 `index.html?page=settings` 与 `index.html?page=help`；
...
- 除根 `index.html`、`Prototype/设计系统.html`、`Prototype/登录.html` 外，全部后台模块通过 `index.html?page=<key>` 进入唯一 App Shell。菜单、原型导航卡片和跨模块入口不得直接打开业务 HTML；业务 HTML 顶层直开时自动回框架，iframe 嵌入时公共脚本不得再次生成壳层。
